...
class Planner:
    ...
    def start_loop(self, agent):
        ...
        while self.simulation_app.is_running(): # 20hz
            ...
            # save depth
            save_path = os.path.join(os.getcwd(), "depth_image"+str(it-start_it)+".png")
            depth_image = infos['observations']['depth_obs'][0,0,:,:].clone().detach().cpu().numpy()
            cv2.imwrite(save_path, depth_image)

            # save rgb
            save_path = os.path.join(os.getcwd(), "rgb_image"+str(it-start_it)+".png")
            rgb_image = infos['observations']['camera_obs'][0,:,:,:3].clone().detach()
            rgb_image_np = rgb_image.cpu().numpy()
            cv2.imwrite(save_path, cv2.cvtColor(rgb_image_np, cv2.COLOR_RGB2BGR))

            # save rgb viz
            save_path = os.path.join(os.getcwd(), "viz_rgb_image"+str(it-start_it)+".png")
            rgb_image = infos['observations']['viz_camera_obs'][0,:,:,:3].clone().detach()
            rgb_image_np = rgb_image.cpu().numpy()
            cv2.imwrite(save_path, cv2.cvtColor(rgb_image_np, cv2.COLOR_RGB2BGR))
